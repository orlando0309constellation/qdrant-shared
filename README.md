...
# MySQL Configuration (optional, for --save, -ml, --latest features)
MYSQL_HOST=localhost
MYSQL_PORT=3306
MYSQL_USER=root
MYSQL_PASSWORD=your_password
MYSQL_DATABASE=qdrant_manager
```
...
**Load from MySQL (display last saved state):**
```bash
qdrant-shard -ls -ml
```

**List and save to MySQL:**
```bash
...
**Use latest data from MySQL (skip querying peers):**
```bash
qdrant-shard -mv -fp 1 -tp 2 --latest
```

**Move and save result to MySQL:**
```bash
...
**Replicate and save result to MySQL:**
```bash
...
|------|-----------|-----------------|-------------|
| `--save` | - | All operations | Save peer information to MySQL after operation |
| `-ml` | `--last-mongo` | `-ls` only | Load and display peer information from MySQL |
| `--latest` | - | `-mv`, `-rs` only | Use latest peer data from MySQL instead of querying |
...
The tool can save and retrieve peer information from MySQL for faster operations and historical tracking.
...
Use the latest MySQL data instead of querying peers (faster for large clusters):

```bash
# Move using latest MySQL data
qdrant-shard -mv -fp 1 -tp 2 --latest

# Replicate using latest MySQL data
qdrant-shard -rs -fp 1 -tp 2 --latest
```
...
# Move with snapshot method and save to MySQL
qdrant-shard -mv -fp 1 -tp 2 --method snapshot --save

# Move using latest MySQL data (faster)
qdrant-shard -mv -fp 1 -tp 2 --latest
```
...
- Connection errors to Qdrant or MySQL
- Operation timeouts
...
If using MySQL features, ensure MySQL is running and configured:
```env
MYSQL_HOST=localhost
MYSQL_PORT=3306
MYSQL_USER=root
